...
export default function WatchlistPage() {
    ...
    const fetchWatchlist = async (signal?: AbortSignal) => {
        ...
        try {
            ...
            if (data) {
                const combined: WatchlistItem[] = [...localData];
                (data as any[]).forEach(serverItem => {
                    if (!combined.find(c => c.coin_id === serverItem.coin_id)) {
    ...
    const fixWatchlistTypes = async (currentWatchlist: WatchlistItem[], signal?: AbortSignal) => {
        // Self-healing: Identify crypto items marked as 'stock' and fix them
        // 1. Get all crypto IDs for checking
        try {
            const query = supabase.from('crypto_coins').select('id');
            if (signal) query.abortSignal(signal);

            const { data: cryptoIds, error } = await query;
            if (signal?.aborted) return;
            if (error) throw error;

            // Combine DB IDs with Fallback IDs for robustness
            const dbIds = (cryptoIds as any[])?.map(c => c.id) || [];
            const cryptoIdSet = new Set([...dbIds, ...FALLBACK_CRYPTO_IDS]);

            let hasUpdates = false;
            const updatedList = currentWatchlist.map(item => {
                // Check if item is marked as stock (or missing type) but exists in crypto list
                const currentType = item.asset_type || 'crypto'; // Default to crypto for check
                if (currentType === 'stock' && cryptoIdSet.has(item.coin_id)) {
                    // It's actually a crypto!
                    hasUpdates = true;
                    const fixedItem = { ...item, asset_type: 'crypto' as const };

                    // Fix in Supabase async - fire and forget, but check abort? 
                    // We can't cancel a fire-and-forget easily unless we await it.
                    // Let's just catch potential errors.
                    (supabase.from('watchlist') as any).update({ asset_type: 'crypto' }).eq('id', item.id).then(({ error }: any) => {
                        if (error) console.error("Auto-fix Supabase update failed", error);
                    }).catch((err: any) => {
                        // Silent catch
                    });
            ...
            if (hasUpdates) {
                ...
                if (!signal?.aborted) {
                    ...
                    // Persist entire updated list to LocalStorage to ensure types are saved
                    if (user?.id) {
            ...
            if (err.name !== 'AbortError') {
                console.error("Error fixing watchlist types", err);
            }
        }
    };
                ...
                (payload: any) => {
                    if (payload.eventType === 'INSERT') {
                        setWatchlist((prev) => {
                            ...
                            return [...prev, payload.new as WatchlistItem];
                        });
            ...
            <div className="grid gap-4">
                ...
                {filteredWatchlist.length === 0 && (
                    <div className="text-center text-muted-foreground py-20 bg-card/60 rounded-2xl border border-dashed border-border">
                        ...
                        <button
                            onClick={() => router.push(activeTab === 'stock' ? '/dashboard/stocks' : '/dashboard/crypto')}
