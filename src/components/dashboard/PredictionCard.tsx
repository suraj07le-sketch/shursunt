...
export function PredictionCard({ pred, isStock, onRepredict }: PredictionCardProps) {
    ...
    const CardContent = ({ pred, isStock, trendColor, statusColor, isBullish }: { pred: Prediction, isStock: boolean, trendColor: string, statusColor: string, isBullish: boolean }) => (
    ...
    if (isMobile) {
        return (
            ...
            >
                ...
                <CardContent pred={pred} isStock={isStock} trendColor={trendColor} statusColor={statusColor} isBullish={isBullish} />
            </motion.div>
        );
    }
